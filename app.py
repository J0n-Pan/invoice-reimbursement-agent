...
def open_archive_folder(folder: Path) -> None:
    """Open a new, visible Windows Explorer window at the archive folder."""
    if os.name != "nt":
        raise ValueError("仅支持在 Windows 本地打开归档文件夹")
    folder = folder.resolve()
    if not folder.is_dir():
        raise ValueError("统一归档文件夹不存在")

    # Passing the directory itself is the most compatible Explorer command on
    # Windows.  It opens the directory even when Explorer is already running,
    # without relying on `/select` or `/root` argument parsing.
    try:
        startupinfo = subprocess.STARTUPINFO()
        startupinfo.dwFlags |= subprocess.STARTF_USESHOWWINDOW
        startupinfo.wShowWindow = 1  # SW_SHOWNORMAL
        subprocess.Popen(["explorer.exe", str(folder)], close_fds=True, startupinfo=startupinfo)
    except OSError as exc:
        try:
            os.startfile(str(folder), "open")
        except OSError as start_error:
            raise ValueError(f"无法打开统一归档文件夹：{start_error}") from exc
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/invoices":
            ...
            return self.send_json({"items": list_invoices(status=status, search=search)})
        if parsed.path.startswith("/api/invoices/") and parsed.path.endswith("/file"):
            job_id = parsed.path.split("/")[3]
            try:
                target = get_invoice_original(job_id)
        ...
        if parsed.path.startswith("/api/invoices/") and parsed.path.endswith("/open-folder"):
            ...
            try:
                target = get_invoice_original(job_id)
                open_archive_folder(target.parent)
                return self.send_json({"message": "已打开统一归档文件夹"})
            except ValueError as exc:
                return self.send_json({"error": str(exc)}, HTTPStatus.NOT_FOUND)
    ...
    def do_DELETE(self) -> None:
        ...
        job_id = parsed.path.rsplit("/", 1)[-1]
        try:
            return self.send_json(delete_invoice(job_id))
        except ValueError as exc:
